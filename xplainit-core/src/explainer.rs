...
impl ExplanationGenerator {
    ...
    pub fn explain(&self, event: &ExecutionEvent) -> String {
        ...
        let main_text = match event {
            ...
            ExecutionEvent::VariableDeclaration {
                name,
                value,
                var_type,
                is_const,
            ...
            ExecutionEvent::DeadlockDetected { threads, .. } => self.explain_deadlock(threads),
    ...
    fn explain_function_exit(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Debug => {
                ...
                format!(
                    "Function {} completed and returned {} (execution time: {:.3}ms)",
                    name,
                    ret_msg,
    ...
    fn explain_variable_declaration(
        &self,
        name: &str,
        value: &Option<Value>,
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "Declared {} {} at {}:{}",
                    keyword, name, location.file, location.line
                );
    ...
    fn explain_variable_assign(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Normal => {
                format!(
                    "Variable {} was assigned the value {}",
                    name,
    ...
    fn explain_conditional_eval(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Normal => {
                format!(
                    "Evaluated condition '{}' which was {}, so taking the '{}' branch",
                    condition,
                    if result { "true" } else { "false" },
                    branch_taken
                )
    ...
    fn explain_loop_entry(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "Entering {} loop at {}:{}",
                    loop_type, location.file, location.line
                );
                if let Some(cond) = condition {
                    msg.push_str(&format!("\n  Condition: {}", cond));
                }
    ...
    fn explain_loop_exit(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Normal => {
                format!(
                    "{} loop completed after {} iterations because {}",
    ...
    fn explain_return(&self, value: &Option<Value>, location: &SourceLocation) -> String {
        match self.verbosity {
            ...
            VerbosityLevel::Normal => {
                if let Some(val) = value {
                    format!(
                        "Returning value {} from current function",
                        self.format_value(val)
                    )
                } else {
    ...
    fn explain_type_error(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "❌ TYPE MISMATCH ERROR at {}:{}\n",
                    location.file, location.line
                );
                msg.push_str(&format!(
                    "  What happened: Tried to {} but the value had the wrong type\n",
                    operation
                ));
                ...
                msg.push_str(&format!(
                    "  💡 Fix: Make sure the value is of type {} before using it in this operation",
                    expected
                ));
                msg
            }
    ...
    fn explain_null_pointer_error(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "❌ NULL POINTER ERROR at {}:{}\n",
                    location.file, location.line
                );
                msg.push_str(&format!(
                    "  What happened: Attempted to {} using variable '{}'\n",
                    operation, variable
                ));
                msg.push_str(&format!(
                    "  Problem: The variable '{}' was null/none/undefined at this point\n",
                    variable
                ));
    ...
    fn explain_index_out_of_bounds(
        &self,
        index: i64,
        size: usize,
        collection: &str,
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "❌ INDEX OUT OF BOUNDS ERROR at {}:{}\n",
                    location.file, location.line
                );
                msg.push_str(&format!(
                    "  What happened: Tried to access element at index {} in collection '{}'\n",
                    index, collection
                ));
                msg.push_str(&format!(
                    "  Problem: The collection only has {} elements\n",
                    size
                ));
                msg.push_str(&format!(
                    "  Valid indices: 0 to {}\n",
                ...
                msg.push_str(
                    "  💡 Fix: Check that your index is within the valid range (0 to length-1)",
                );
                msg
            }
        }
    ...
    fn explain_division_by_zero(
        ...
        match self.verbosity {
            VerbosityLevel::Brief => "❌ Division by zero".to_string(),
            VerbosityLevel::Normal => {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                ...
                msg.push_str(&format!(
                    "  What happened: Attempted to divide {} by zero\n",
                    self.format_value(numerator)
                ));
                if let Some(var) = denominator_var {
                    msg.push_str(&format!(
                        "  Problem: The denominator variable '{}' had the value 0\n",
                        var
                    ));
                    msg.push_str(&format!(
                        "  💡 Fix: Check that '{}' is not zero before dividing",
                        var
                    ));
                } else {
    ...
    fn explain_stack_overflow(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "❌ STACK OVERFLOW ERROR at {}:{}\n",
                    location.file, location.line
                );
                msg.push_str(&format!(
                    "  What happened: Function '{}' called itself recursively {} times\n",
                    function, recursion_depth
                ));
                ...
                msg.push_str(&format!(
                    "  💡 Fix: Review the base case in '{}' and ensure it can be reached",
                    function
                ));
                msg
            }
    ...
    fn explain_infinite_loop(
        ...
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                let mut msg = format!(
                    "⚠️  INFINITE LOOP DETECTED at {}:{}\n",
                    location.file, location.line
                );
                msg.push_str(&format!(
                    "  What happened: The {} loop has executed {} iterations\n",
                ...
                msg.push_str(
                    "    2. The variables in the condition aren't being updated correctly\n",
                );
                msg.push_str("    3. You forgot to include a break or return statement\n");
                msg.push_str(
                    "  💡 Fix: Review the loop condition and ensure it can eventually become false",
                );
                msg
            }
        }
    ...
    fn explain_deadlock(&self, threads: &[String]) -> String {
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                ...
                msg.push_str(&format!(
                    "  What happened: {} threads are stuck waiting for each other\n",
                    threads.len()
                ));
                ...
                msg.push_str(
                    "  Problem: Each thread is waiting for a resource held by another thread\n",
                );
    ...
    fn explain_memory_leak(&self, allocation_count: usize, leaked_bytes: usize) -> String {
        match self.verbosity {
            ...
            VerbosityLevel::Detailed | VerbosityLevel::Debug => {
                ...
                msg.push_str(&format!(
                    "  Total leaked memory: {} bytes ({:.2} KB)\n",
                    leaked_bytes,
