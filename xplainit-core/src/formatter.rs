    ...
    fn format_event(&self, event: &ExecutionEvent) -> String {
        ...
        match event {
            ExecutionEvent::FunctionEnter { name, location, .. } => {
                let _ = writeln!(
                    html,
                    "    <p><strong>Calling function:</strong> {}</p>",
                    name
                );
                let _ = writeln!(
                    html,
            ...
            ExecutionEvent::FunctionExit {
                ...
                let _ = writeln!(
                    html,
                    "    <p><strong>Function returned:</strong> {}</p>",
                    name
                );
                ...
                let _ = writeln!(
                    html,
                    "    <p class=\"duration\">Duration: {:.3}ms</p>",
...
impl FormatterFactory {
    pub fn create(format: OutputFormat, verbosity: VerbosityLevel) -> Box<dyn OutputFormatter> {
        match format {
            ...
            OutputFormat::Markdown => Box::new(MarkdownFormatter::new(verbosity)),
        }
    }
}
