...
impl PerformanceFilter {
    ...
    pub fn is_hot_event(&self, event: &ExecutionEvent) -> bool {
        ...
        match event {
            ...
            | ExecutionEvent::FunctionExit { name, .. } => {
...
impl AdvancedFilter {
    ...
    pub fn should_capture(&mut self, event: &ExecutionEvent, thread_id: &str) -> bool {
        ...
        if !self
            .call_stack_filter
            .should_capture_at_depth(thread_id, event)
        {
            return false;
        }
...
mod tests {
    ...
    fn test_regex_filter_include() {
        let filter = RegexFilter::new().include_pattern(r"^test_.*").unwrap();
    ...
    fn test_regex_filter_exclude() {
        let filter = RegexFilter::new().exclude_pattern(r"^_.*").unwrap();
